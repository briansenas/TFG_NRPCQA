...
\begin{frame}
  \frametitle{Modelo NR3DQA\footnotemark[10]}
\begin{table}[htp]
  \small
  \begin{center}
    \hspace{-.5cm}
  ...
  \caption[Resultados de prueba preliminar con NR3DQA.]{
    Resultados de prueba preliminar con NR3DQA\footnotemark[10]. 
  }
...
autores. Logramos obtener resultados similares.
...
En la tabla podéis ver los mejores resultados, 
...
\begin{table}[htp]
  ...
  \caption[Resultados de experimento preliminar VQA-PC en SJTU.]{
    Resultados de experimento preliminar VQA-PC\footnotemark[11]. 
  }
  \label{tab:PreTestResults}
\end{table}
\begin{enumerate}
  \item Las predicciones del modelo no tienen porque estar en la misma escala.
  \item Seria necesario utilizar regresión logística-5 para normalizar la escala. 
  \item Pero podemos centrarnos en el \textbf{SROCC} que es \textbf{invariante a la escala}.
\end{enumerate}
\footnotetext[11]{\cite{VQA-PC}}
\end{frame}
...
superar la barrera del 90\% en la mediana de los métodos F0 a F3.
Gracias a la información adicional hemos logrado valores superiores, llegando a 
obtener una correlación del 88\% de media, con mediana al 94\%, utilizando el modelo 
F2 de fusión por convolución. 
}
