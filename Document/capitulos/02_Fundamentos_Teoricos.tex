...
adyaciente con distancia horizontal igual a 1 hasta llegar al final de esa fila 
...
para obtener información de movimiento, ver (\ref{fig:SlowFastPathways}). 

\begin{figure}[htp]
  \begin{center}
    \includegraphics[width=0.5\textwidth]{imagenes/chapter2/SlowFastPathways.png}
  \end{center}
  \caption{Ejemplo extraído de\cite{SlowFastNetworks} para ilustrar la distinción 
...
\begin{figure}[htp]
  ...
  \caption{Ejemplo extraído de\cite{VoxelizationExample} que demuestra el resultado 
...
proyectados sobre las imágenes (\emph{ray casting}), ver (\ref{fig:CT2Volume}).

\begin{figure}[htp]
  \centering 
  \includegraphics[width=\textwidth]{imagenes/chapter2/DicomDistortionsExample.png}
  \caption{Ejemplo de artefactos sobre imágenes Dicom\cite{DicomDistortionsExample}: 
  (a) un artefacto de anilo, (b) un difuminado provocado por movimiento, 
...
artefactos luminosos y problemas de interpolación o ruido al generar la proyección (\ref{fig:DicomDistortionsExample}). 
...
Dos de ellas serán para simular los resultados de varios algoritmos de compression, 
como puede ser \emph{octree compression}\cite{OctreeCompression} y la reducción de número de puntos 
por medio de mostreo uniforme (\emph{uniform downsampling}), que suele ser el paso 
...
como el movimiento del paciente y aquellos provocados por ruido en la transmissión de datos.
...
\begin{figure}[htp]
  ...
  \end{subfigure}
  \caption{Ejemplo de distorsiones generadas. Leyendo de izquierda a derecha tenemos:
    la imagen original, compressión \emph{octree}, reducción de puntos por \emph{uniform downsampling},
